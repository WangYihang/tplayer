...
def play(vidoe_path):
    ...
    while success:
        ...
        clear_screen()
        print show_ascii(im, get_current_size(im.size))
...
def get_current_size(img_size):
    ...
    if img_scale > termial_scale:
        current_size = (termial_size[0], int(img_size[1] * (termial_size[0] * 1.0 / img_size[0])) * 2)
    else:
        current_size = (int(img_size[0] * (termial_size[1] * 1.0 / img_size[1])) * 2, termial_size[1])
    return current_size
...
def main():
    ...
    try:
        ...
        print show_ascii(im, get_current_max_size(im.size))
        exit(0)
